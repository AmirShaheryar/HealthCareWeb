...
def generate_soap(text):
    ...
    cond_map = {
        ...
        for keyword, diagnosis in cond_map.items():
        if keyword in text_lower:
            conditions.append(diagnosis)
    assessment = "; ".join(conditions[:4]) if conditions else "Assessment requires further clinical evaluation."

    # Plan
    plan_items = []
    med_match = re.search(r'(metformin|paracetamol|ibuprofen|aspirin|insulin|lisinopril)[^\s]*\s*([\d.]+\s*mg)?', text_lower)
    if med_match: plan_items.append(f"Continue/review: {med_match.group(0).strip()}")
    if "rest" in text_lower: plan_items.append("Advise adequate rest")
    if "follow" in text_lower: plan_items.append("Schedule follow-up appointment")
    if "blood test" in text_lower or "hba1c" in text_lower or "glucose" in text_lower:
        plan_items.append("Order blood work / HbA1c")
    plan_items.append("Reinforce patient education and lifestyle modifications")
    plan = "; ".join(plan_items)

    soap = f"""<b>SOAP NOTE — NLP-Medora AI Generated</b>0
{'─'*50}

<b>PATIENT:</b>  {age}-year-old {gender}
<b>DATE:</b>     {datetime.now().strftime('%Y-%m-%d')}
<b>GENERATED BY:</b> NLP-Medora v1.0
...
{'─'*50}
⚠️  This note is AI-generated from free text. 
Clinician must review, verify, and countersign before use.
"""
    return soap
