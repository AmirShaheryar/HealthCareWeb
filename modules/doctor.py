...
from modules.db import add_clinical_note
...
def show():
    ...
    def _is_assigned_to_me(req: dict) -> bool:
        assigned_email = req.get("assigned_doctor_email", "")
        if assigned_email:
            return assigned_email == my_email
        return not req.get("assigned_doctor")

    def _save_and_share_note(note_payload: dict):
        """Persist doctor notes and keep session mirror for current run."""
        add_clinical_note(note_payload)
        st.session_state.shared_notes.append(note_payload)

    pending   = [r for r in reqs if r["status"] == "Pending" and _is_assigned_to_me(r)]
    reviewed  = [
        r for r in reqs
        if r["status"] in ["Verified", "Rejected"]
        and (
            _is_assigned_to_me(r)
            or r.get("doctor_name") == st.session_state.get("username")
        )
    ]
        ...
        if not pending:
            ...
            for req in pending:
                with st.expander(f"{req['id']} — {req['patient_name']} — {req['ai_prediction']}"):
                    ...
                    if req.get("assigned_doctor"):
                        st.write(f"**Assigned Doctor:** {req['assigned_doctor']}")
...
""",
                                ...
                                _save_and_share_note(note_payload)
                                break
